...
def fig_margin_geometry():
    ...
    ax.annotate("", xy=pw + u * 1.0, xytext=pw,
                ...
                zorder=5)
    ax.text(pw[0] + 1.0, pw[1] + .30,
            "$w$ = vector pháp tuyến\n(vuông góc siêu phẳng)",
            color=C4, fontsize=9.5, fontweight="bold", va="center",
    ...
    ax.annotate(r"độ rộng margin $=\dfrac{2}{\|w\|}=%.2f$" % (2 / nw),
                xy=tuple(pm), xytext=(5.35, 0.35), color=C3, fontsize=11,
    ...
    ax.set_xlim(-0.4, 9.0); ax.set_ylim(-0.5, 7.0)
...
def fig_why_large_margin():
    ...
    ax.annotate("Ngôi sao = điểm TEST mới (thật sự là lớp $+1$)\n"
                "Nó rơi vào KHE giữa hai đường:\n"
                "• đường C  → đoán lớp $-1$  → SAI\n"
                "• SVM      → đoán lớp $+1$  → ĐÚNG",
                xy=tuple(test), xytext=(4.35, 1.15), fontsize=9.5,
    ...
    ax.set_xlim(0, 8.4); ax.set_ylim(0, 7.9)
    ax.legend(fontsize=8.5, loc="upper left")
    ax.set_xlabel("$x_1$"); ax.set_ylabel("$x_2$")
...
def fig_hard_vs_soft():
    ...
    axes[1].scatter(*Xo[-1], marker="*", s=430, color=C2, edgecolor="k",
                    linewidth=1.1, zorder=6)
    axes[1].annotate("outlier", xy=Xo[-1], xytext=(0.3, 5.4), fontsize=9.5,
    ...
    axes[2].text(0.25, 5.7,
                 r"$y_i(w^Tx_i+b)\geq 1-\xi_i,\;\xi_i\geq 0$" "\n"
                 r"phạt $C\sum_i \xi_i$ trong hàm mục tiêu",
                 fontsize=9, bbox=dict(boxstyle="round,pad=.35", fc="#fff7ed", ec=C4))
...
def fig_effect_of_C():
    ...
    box = dict(boxstyle="round,pad=.35", fc="white", ec="lightgray", alpha=.92)
    axes[0].text(.03, .03, "C nhỏ → khoan dung với vi phạm\nmargin RỘNG, NHIỀU support vector\n"
                 "(bias cao, variance thấp)",
                 transform=axes[0].transAxes, fontsize=8.5, color="dimgray",
                 va="bottom", bbox=box)
    axes[1].text(.03, .03, "C vừa → cân bằng\n(mặc định của sklearn là C = 1)",
                 transform=axes[1].transAxes, fontsize=8.5, color="dimgray",
                 va="bottom", bbox=box)
    axes[2].text(.03, .03, "C lớn → khắt khe, cố gò sát dữ liệu\nmargin HẸP, ÍT support vector\n"
                 "(bias thấp, variance cao → dễ overfit)",
                 transform=axes[2].transAxes, fontsize=8.5, color="dimgray",
                 va="bottom", bbox=box)
...
def fig_kernel_trick_3d():
    ...
    fig.text(.435, .60, r"$\phi$", fontsize=22, color=C4, fontweight="bold",
             ha="center", va="center")
    ax.annotate("", xy=(.478, .52), xytext=(.392, .52), xycoords="figure fraction",
                arrowprops=dict(arrowstyle="-|>", color=C4, lw=3, mutation_scale=22))
    fig.text(.435, .455, "nâng chiều", fontsize=9.5, color=C4, ha="center", va="top")

    ax = fig.add_axes([0.47, 0.03, 0.52, 0.76], projection="3d")
...
def fig_gamma_rbf():
    ...
    for ax, g, note in zip(axes, [0.1, 1, 10, 100], notes):
        ...
        ax.text(.03, .03, note, transform=ax.transAxes, fontsize=8.2,
                color="dimgray", va="bottom",
                bbox=dict(boxstyle="round,pad=.3", fc="white", ec="lightgray", alpha=.9))
        ax.set_xlabel("$x_1$"); ax.set_ylabel("$x_2$")
...
def fig_compare_kernels():
    ...
    for ax, (kw, name) in zip(axes, cfg):
        ...
        ax.set_xlabel("$x_1$ (đã scale)"); ax.set_ylabel("$x_2$ (đã scale)")
...
def fig_hinge_loss():
    ...
    ax.step(m, zero_one, where="post", color="k", lw=2.4,
            label="0-1 loss (cái ta THỰC SỰ muốn tối thiểu)")
    ...
    ax.plot(m, logistic, color=C1, lw=2,
            label=r"log loss $\log_2(1+e^{-m})$ — Logistic Regression")
    ax.axvline(0, color="gray", lw=1); ax.axvline(1, color=C3, lw=1.2, ls=":")
    ax.text(1.12, 1.42, "$m=1$: mép lề", color=C3, fontsize=9)
    ax.set_xlabel(r"functional margin $m = y\,(w^Tx+b)$")
    ax.set_ylabel("mất mát")
    ax.set_ylim(-.15, 3.2); ax.legend(fontsize=8.5)
    ...
    ax.axvline(1, color=C3, lw=1.2, ls=":")
    ax.annotate("Hinge = 0 HOÀN TOÀN khi $m\\geq1$\n"
                "→ điểm nằm ngoài lề KHÔNG đóng góp gradient\n"
                "→ nghiệm chỉ phụ thuộc support vector (mô hình THƯA)",
                xy=(1.9, .02), xytext=(-.55, 1.35), fontsize=9,
                arrowprops=dict(arrowstyle="->", color=C3),
                bbox=dict(boxstyle="round,pad=.4", fc="#ecfdf5", ec=C3))
    ax.annotate("Log loss > 0 với MỌI điểm\n→ mọi mẫu đều kéo nghiệm một chút\n(mô hình KHÔNG thưa)",
                xy=(2.4, logistic[np.argmin(np.abs(m - 2.4))]), xytext=(.45, 2.5),
    ...
    ax.set_ylim(-.15, 3.2); ax.legend(fontsize=9)
...
def fig_rbf_similarity():
    ...
    ax.text(3.05, .53, "mức 'giống nhau một nửa'", fontsize=8, color="gray")
...
def fig_svr_epsilon_tube():
    ...
    ax.set_xlabel("x"); ax.set_ylabel("y"); ax.legend(fontsize=8.5, loc="upper left")
